...
界面仍为 54 格容器，顶部为头盔、胸甲、护腿、靴子、副手、主手六格，其后为隔断、信息和一键拾取。主手只在顶栏显示，原快捷栏位置留空，原槽位恢复关系保留。信息按钮左键显示详情及剩余经验，有额外掉落时右键循环翻页。拾取按钮正常点击领取物品与经验，按 Q 只领取经验。背景图片不会改变点击位置，下方玩家背包保持原版。
...
`{default}` 使用当前语言名称，Lore 中单独一行 `{details}` 展开动态说明（包括经验领取提示）；信息按钮显示死亡者、时间、位置、物品格数和经验。设为空列表可隐藏说明。名称与 Lore 支持 `&` 颜色代码。CustomModelData 为 0 时保留原版图标。内置编号 7310200 用于黑色玻璃分隔板：装包后透明显示，让裂纹石框连贯；未装包时保留原版玻璃。其他编号需与自己的模型对应。
...
这些配置适用于所有遗体外观，位于 `config.yml`：

```yaml
language: zh_CN # 或 en_US；自定义文件放到 languages/ 下
death:
  skip-keep-inventory: true
  exclude-items:
    name-contains: []
...
默认尊重死亡不掉落。关闭跳过设置后也不复制玩家保留的背包，仅可接管确实失去的经验。排除规则忽略颜色代码和大小写，包含任一文字即匹配；命中物品交回死亡掉落或绑定插件处理，不会由本插件返还。Paper 的保留物品清单同样优先。

经验保存比例为 0–100，默认 50，向下取整。0 不接管经验，保留原有经验掉落；100 保存全部经验点。其他插件设置保留经验、重生经验或改变原版经验掉落时优先由其处理。例如完整 6 级为 72 点，50% 保存 36 点，约合从 0 级升至 3 级并保留部分进度。

编辑语言文件或以上设置后执行 `/td reload`。既有遗体保留已保存的经验数量，新比例只影响后续死亡。已有语言文件和墓碑按钮自定义不会被覆盖；如要让按钮随语言变化，保留 `{default}` 与 `{details}`。
