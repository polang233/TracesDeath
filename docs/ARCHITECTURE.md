...
按职责分为 `commands`、`core`、`config`、`corpse`、`gui`、`storage`、`entity`、`compat`、`resourcepack`、`language`、`experience` 和 `hook`。主类负责启动装配和停用清理，各包负责对应业务。
...
- `language/Messages`：按运行实例加载 UTF-8 语言文件，缺失键使用内置翻译，重载失败保留旧语言。
- `experience/Experience`：等级与进度条换算经验点，保存精确领取前后经验状态。
- `config/DeathSettings`：死亡不掉落、名称／Lore 排除规则与经验保留比例。
- `corpse/Corpse`：保存遗体身份、世界与位置、玩家皮肤、死亡时间、物品、经验点和待完成领取。
...
`corpses/<UUID>.yml` 当前格式为 schema 4，兼容 schema 1–3；旧记录经验默认为 0。皮肤以 name/value/signature 属性列表保存，读取旧 Profile 时提取属性。物品仍使用对应服务端的 ItemStack 格式，高版本物品数据不能保证降级读取。死亡时间缺失时显示“未记录”；旧的 36 格领取快照按原范围核对，新记录覆盖背包、护甲、副手共 41 格。记录不保存实体 UUID。
...
`CorpseItems` 先计算目标库存，再由 `CorpseService` 保存 `PendingClaim`。记录包含领取者、库存范围、领取前后快照、遗体剩余物品、计划掉落物、经验领取数量、领取前后经验状态和 `drops-started`。
...
保存领取计划成功后才修改玩家库存和经验并调用玩家保存。若有地面掉落，先持久化 `drops-started` 再生成物品，最后提交遗体结果。界面关闭后同步最终玩家库存。

保存失败时保留待完成记录，锁定对应遗体及领取者的后续领取。登录时同时核对库存及涉及领取的经验状态，避免 Q 领取前后库存相同而误判。依据快照恢复：与领取后相同则继续完成，与领取前相同则保留原库存；无法判断或已进入掉落阶段时交给管理员核对。

人工核对使用控制台 `td recover <UUID> delivered` 或 `td recover <UUID> not-delivered`，领取玩家必须离线。命令只记录审核结果，不修改玩家背包或经验，也不补发地面物品。
...
`config.yml` 管理语言、遗体类型、死亡排除、经验与领取规则。`languages/zh_CN.yml` 和 `en_US.yml` 首次启动时释放，选中语言随 `/td reload` 生效。选择墓碑后，`tombstone.yml` 管理模型、自定义 GUI 和测试地址；其他类型不访问该文件。入口校验后向服务传入解析结果。`/td reload` 先严格解析 YAML、检查配置与版本、验证遗体文件，再关闭当前界面、取消旧任务与监听并重建服务。准备阶段失败保留现有实例；启用阶段失败则按旧配置从已保存记录恢复，回退也失败时停用插件并保留记录。bStats 实例跨配置重载保留。客户端资源包需自行重新加载或手动测试发送。
...
死亡监听在 HIGHEST 处理当前实际掉落，保留未接管条目。默认跳过 `keepInventory`；关闭 `death.skip-keep-inventory` 后，只允许在确实失去经验时生成经验遗体，仍不收集保留背包内的物品。名称／Lore 排除匹配忽略颜色与大小写；Paper `getItemsToKeep()` 中的物品也不接管。下一 tick 展示前会再次核对取消、保留标记、Paper 保留物品及重新加入的掉落。

插件无法推断其他插件在重生后私下返还哪些物品。这类绑定插件应配置排除标记；排除只表示不进入遗体，不负责返还。事件优先级相同的插件仍有执行顺序差异，上线前应以实际绑定插件验证。

经验以当前等级和进度条计算，不使用可能过期的 `getTotalExperience()` 作为余额。保留比例向下取整，默认 50%；0 不接管经验，保持原有结算。`keepLevel`、自定义重生经验或非原版死亡经验掉落已生效时，本插件不接管经验。纯经验遗体也能创建和领取。

正常点击领取按钮收集物品与经验，Q 只领取经验，单格领取最后一件物品时附带剩余经验。信息按钮的约合等级是从 0 级累计这些经验点能达到的等级，并非领取者必定提升的等级。只有物品与经验均为空才清理遗体。

经验与物品共用领取日志、玩家存档保存与恢复判断。混合或无法识别的存档状态锁定待审核，避免再次发放；并不保证任意外部存档回滚后的自动恢复。死亡接管保存与服务端死亡结算也存在跨文件保存边界。
