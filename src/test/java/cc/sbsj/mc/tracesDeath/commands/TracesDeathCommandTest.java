...
class TracesDeathCommandTest {
    @Test void recoveryIsRestrictedToConsoleEvenForAnAdminPlayer() {
        CorpseService service = mock(CorpseService.class);
        Player player = mock(Player.class);
        when(player.hasPermission("tracesdeath.admin")).thenReturn(true);
        new TracesDeathCommand(service).onCommand(player, null, "td",
                new String[]{"recover", UUID.randomUUID().toString(), "delivered"});
        verifyNoInteractions(service);
    }

    @Test void consoleRecoveryDelegatesTheExplicitOutcome() throws Exception {
        CorpseService service = mock(CorpseService.class);
        ConsoleCommandSender console = mock(ConsoleCommandSender.class);
        UUID id = UUID.randomUUID();
        TracesDeathCommand command = new TracesDeathCommand(service);
        command.onCommand(console, null, "td", new String[]{"recover", id.toString(), "delivered"});
        verify(service).resolveClaim(id, true);
        command.onCommand(console, null, "td", new String[]{"recover", id.toString(), "not-delivered"});
        verify(service).resolveClaim(id, false);
    }

    @Test void invalidRecoveryOutcomeDoesNotChangeState() {
        CorpseService service = mock(CorpseService.class);
        ConsoleCommandSender console = mock(ConsoleCommandSender.class);
        new TracesDeathCommand(service).onCommand(console, null, "td",
                new String[]{"recover", UUID.randomUUID().toString(), "unknown"});
        verifyNoInteractions(service);
        verify(console).sendMessage(contains("恢复失败"));
    }

    @Test void defaultListFiltersOwnershipAndAdminCanSeeAll() {
        CorpseService service = mock(CorpseService.class);
        Player player = mock(Player.class);
    ...
    private Corpse corpse(UUID owner) {
        ...
        return new Corpse(UUID.randomUUID(), owner, "Player", UUID.randomUUID(), 0, 64, 0,
                0, 0, profile, 0, Map.of());
    }
}
