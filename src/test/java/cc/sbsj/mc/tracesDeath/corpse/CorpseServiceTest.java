...
class CorpseServiceTest {
    ...
    private void recovery(boolean delivered, boolean ambiguous) throws Exception {
        ...
        f.corpse.begin(new Corpse.PendingClaim(f.corpse.owner, 36, Map.of(), after, Map.of(), List.of(), false));
    ...
    @Test void restoringEquipmentJournalsDisplacedItemBeforeDroppingIt() throws Exception {
        Fixture f = new Fixture();
    ...
    @Test void interruptedWorldDropsStayLockedInsteadOfBeingReplayedOnLogin() throws Exception {
        Fixture f = new Fixture();
        f.corpse.begin(new Corpse.PendingClaim(f.corpse.owner, 41, Map.of(), Map.of(), Map.of(),
                List.of(CorpseItemsTest.item(Material.DIAMOND, 3)), true));
        try (var bukkit = mockStatic(Bukkit.class)) {
            bukkit.when(Bukkit::getPluginManager).thenReturn(mock(PluginManager.class));
            bukkit.when(Bukkit::getScheduler).thenReturn(mock(BukkitScheduler.class));
            bukkit.when(Bukkit::getOnlinePlayers).thenReturn(List.of());
    ...
    @Test void manualResolutionRequiresTheRecipientToBeOffline() throws Exception {
        Fixture f = new Fixture();
        f.corpse.begin(new Corpse.PendingClaim(f.corpse.owner, 41, Map.of(), Map.of(), Map.of(), List.of(), false));
        try (var bukkit = mockStatic(Bukkit.class)) {
            bukkit.when(() -> Bukkit.getPlayer(f.corpse.owner)).thenReturn(f.player);
            assertThrows(IllegalArgumentException.class, () -> f.service.resolveClaim(f.corpse.id, true));
            verify(f.store, never()).save(any());
    ...
    private static final class Fixture {
        ...
        Fixture() throws Exception {
            ...
            service = new CorpseService(plugin, store, false, false);
        }
    }
}
