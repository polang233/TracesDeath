...
class CorpseMenuTest {
    @Test void informationIncludesIdentityTimeAndLocation() {
    ...
    @Test void legacyRecordReportsUnknownDeathTime() {
        Corpse original = CorpseTest.corpse();
        Corpse legacy = new Corpse(original.id, original.owner, original.name, original.world,
                original.x, original.y, original.z, original.yaw, original.heldSlot, original.profile, 0, original.items());
        try (var ignored = mockStatic(Bukkit.class)) {
