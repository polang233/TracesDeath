...
class CorpseEntitiesTest {
    @Test void preciseClickOpensWhenGenericListenerRunsFirst() throws Exception {
    ...
    @Test void preciseClickOpensWhenPreciseListenerRunsFirst() throws Exception {
        Fixture f = new Fixture();
        var event = f.precise(EquipmentSlot.HAND);
        f.precise().callEvent(event);
        f.generic().callEvent(event);
    ...
    @Test void genericClickStillOpensOnVersionsWithSeparateEventLists() throws Exception {
        Fixture f = new Fixture();
    ...
    @Test void offhandDoesNotOpenAndProtectionCancellationIsRespected() throws Exception {
        Fixture f = new Fixture();
    ...
    @Test void unrelatedEntitiesRemainUnaffected() throws Exception {
        Fixture f = new Fixture();
        when(f.data.get(any(NamespacedKey.class), eq(PersistentDataType.STRING))).thenReturn(null);
        var event = f.precise(EquipmentSlot.HAND);
        f.generic().callEvent(event);
        f.precise().callEvent(event);
    ...
    private static final class Fixture {
        ...
        Fixture() {
            ...
            when(data.get(any(NamespacedKey.class), eq(PersistentDataType.STRING))).thenReturn(id.toString());
            listener = new CorpseEntities(plugin, Map.of(), open, ignored -> {});
        }
        ...
        RegisteredListener generic() {
            return new RegisteredListener(listener,
