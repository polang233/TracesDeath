...
public final class ResourcePackTestService implements AutoCloseable {
    ...
    public void exportPacks() throws Exception {
        ...
        for (String suffix : new String[] {"", "-1.19.4"}) {
            String filename = "tracesdeath" + suffix + ".zip";
            byte[] bytes;
    ...
    public void send(Player player) throws Exception {
        if (!version.atLeast(1, 19, 4))
            throw new IllegalArgumentException("内置材质包测试需要 Minecraft 1.19.4+");
        ...
        String filename = "tracesdeath" + (version.atLeast(1, 20, 0) ? "" : "-1.19.4") + ".zip";
